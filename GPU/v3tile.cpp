...
int main(int argc, char **argv)
{
    ...
    cl::sycl::range<3> local_epi(1, block_snp, 1);
    ...
    for(int i0 = 0; i0 < num_snp; i0 += block_sched)
    {
        for(int i1 = i0; i1 < num_snp; i1 += block_sched)
        {
            for(int i2 = i1; i2 < num_snp; i2 += block_sched)
            {
                ...
                //while(1)
                //{
                //    if(event_kernel.get_info<cl::sycl::info::event::command_execution_status>() == cl::sycl::info::event_command_status::complete)
                //    {
    ...
    /* queue.submit([&](cl::sycl::handler& h)
    {
        h.parallel_for<class kernel_red>(cl::sycl::nd_range<1>(global_red, local_red), [=](cl::sycl::nd_item<1> id)
        {
            ...
            int size = block_sched * block_sched * block_sched;
