...
{
    int num_snp, num_snp_m, num_pac, phen_ones, dim_epi, block_snp, x;
    ...
    queue.submit([&](cl::sycl::handler& h)
    {
        h.parallel_for<class kernel_epi>(cl::sycl::nd_range<3>(global_epi, local_epi), [=](cl::sycl::nd_item<3> id)
        {
            ...
            if(j > i && t > j && t < num_snp)
            {
                ...
                // Phenotype 0
                uint* SNPi = &dev_data_zeros[i * 2];
                uint* SNPj = &dev_data_zeros[j * 2];
                uint* SNPt = &dev_data_zeros[t * 2];
                ...
                // Phenotype 1
                SNPi = &dev_data_ones[i * 2];
                SNPj = &dev_data_ones[j * 2];
                SNPt = &dev_data_ones[t * 2];
